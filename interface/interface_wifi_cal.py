...
def sphe2cart (r, theta, phi):
    ...
    theta = np.deg2rad(theta+26)+m.atan2(14.5,r)
    phi = np.deg2rad(phi-6)
    x = np.cos(theta) * np.sin(phi) * (r + abs(30.5*np.cos(theta)))
    y = np.sin(phi) * np.sin(theta) * (r + abs(30.5*np.sin(theta)))
    z = np.cos(phi) * r
